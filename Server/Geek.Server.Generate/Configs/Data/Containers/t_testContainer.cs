                    ...
                    foreach (var d in list)
                    {
                        if (!map.ContainsKey(d.t_id))
                            map.Add(d.t_id, d);
                             ...
                             LOGGER.Error("Exist duplicate Key: " + d.t_id + " t_testBean");
                    }
				}
				catch (Exception ex)
				{
					 LOGGER.Error("import data error: t_testBean >>" + ex.ToString());
				}
			}
			else
			{
				 LOGGER.Error("can not find conf data: t_testBean.bytes");
			}
		}
