                    ...
                    foreach (var d in list)
                    {
                        if (!map.ContainsKey(d.t_id))
                            map.Add(d.t_id, d);
                             ...
                             LOGGER.Error("Exist duplicate Key: " + d.t_id + " t_test2Bean");
                    }
				}
				catch (Exception ex)
				{
					 LOGGER.Error("import data error: t_test2Bean >>" + ex.ToString());
				}
			}
			else
			{
				 LOGGER.Error("can not find conf data: t_test2Bean.bytes");
			}
		}
