    ...
    internal sealed partial class NetworkManager : GameFrameworkModule, INetworkManager
    {
        ...
        private sealed class TcpWithSyncReceiveNetworkChannel : NetworkChannelBase
        {
            ...
            public override void Connect(IPAddress ipAddress, int port, object userData)
            {
                ...
                m_Socket = new SystemNetSocket(ipAddress.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
                if (m_Socket == null)
                {
            ...
            {
                base.ProcessReceive();
                while (((SystemNetSocket)m_Socket).Available > 0)
                {
                    if (!ReceiveSync())
                    {
                        break;
            ...
            private void ConnectAsync(IPAddress ipAddress, int port, object userData)
            {
                try
                {
                    ((SystemNetSocket)m_Socket).BeginConnect(ipAddress, port, m_ConnectCallback, new ConnectState(m_Socket, userData));
                }
            ...
            private void SendAsync()
            {
                try
                {
                    ((SystemNetSocket)m_Socket).BeginSend(m_SendState.Stream.GetBuffer(), (int)m_SendState.Stream.Position, (int)(m_SendState.Stream.Length - m_SendState.Stream.Position), SocketFlags.None, m_SendCallback, m_Socket);
                }
                catch (Exception exception)
                {
                    m_Active = false;
            ...
            private bool ReceiveSync()
            {
                try
                {
                    int bytesReceived = ((SystemNetSocket)m_Socket).Receive(m_ReceiveState.Stream.GetBuffer(), (int)m_ReceiveState.Stream.Position, (int)(m_ReceiveState.Stream.Length - m_ReceiveState.Stream.Position), SocketFlags.None);
                    if (bytesReceived <= 0)
                    {
                        Close();
                        return false;
