...
namespace GameFramework.Network
{
    internal sealed partial class NetworkManager : GameFrameworkModule, INetworkManager
    {
        private sealed class ReceiveState : IDisposable
        {
            ...
            public void PrepareForPacketHeader(int offset,int packetHeaderLength)
            {
                Reset(offset,packetHeaderLength, null);
            }
            ...
            public void PrepareForPacket(int offset,IPacketHeader packetHeader)
            {
                if (packetHeader == null)
                {
                    throw new GameFrameworkException("Packet header is invalid.");
                }
                Reset(offset,packetHeader.PacketLength, packetHeader);
            }
            public void PrepareForPacket(int offset,int targetLength,IPacketHeader packetHeader)
            {
                if (packetHeader == null)
                {
                    throw new GameFrameworkException("Packet header is invalid.");
                }
                Reset(offset,targetLength, packetHeader);
            }
            ...
            private void Reset(int offset, int targetLength, IPacketHeader packetHeader)
            {
                if (targetLength < 0)
                {
                    throw new GameFrameworkException("Target length is invalid.");
