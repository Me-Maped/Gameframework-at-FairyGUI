        ...
        public void ReadData(string dataAssetName, int priority, object userData)
        {
            ...
            switch (result)
            {
                ...
                case HasAssetResult.AssetOnFileSystem:
                    m_ResourceManager.LoadAssetAsync(dataAssetName, m_LoadAssetCallbacks, userData);
                    break;
                case HasAssetResult.BinaryOnDisk:
                    throw new GameFrameworkException(Utility.Text.Format("Could Not Load Data asset '{0}' is '{1}'!", dataAssetName, result));
