...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class WebRequestComponent : GameFrameworkComponent
    {
        ...
        private WebRequestAgentHelperBase m_DefaultWebRequestAgentHelper = null;
        ...
        {
            m_DefaultWebRequestAgentHelper.SetRequestHeader(name, value);
        }
        ...
        {
            m_DefaultWebRequestAgentHelper.ClearRequestHeader();
        }
        ...
        {
            m_DefaultWebRequestAgentHelper = Helper.CreateHelper(m_WebRequestAgentHelperTypeName, m_CustomWebRequestAgentHelper, index);
            if (m_DefaultWebRequestAgentHelper == null)
            {
                Log.Error("Can not create web request agent helper.");
                return;
            }

            m_DefaultWebRequestAgentHelper.name = Utility.Text.Format("Web Request Agent Helper - {0}", index);
            Transform transform = m_DefaultWebRequestAgentHelper.transform;
            transform.SetParent(m_InstanceRoot);
            transform.localScale = Vector3.one;

            m_WebRequestManager.AddWebRequestAgentHelper(m_DefaultWebRequestAgentHelper);
        }
        ...
        {
            Log.Info("Web request start, web request serial id '{0}', web request uri '{1}'.", e.SerialId, e.WebRequestUri);
        ...
        {
            Log.Info("Web request success, web request serial id '{0}', web request uri '{1}', data '{2}'.", e.SerialId, e.WebRequestUri, System.Text.Encoding.UTF8.GetString(e.GetWebResponseBytes()));
        ...
        {
            Log.Error("Web request failure, web request serial id '{0}', web request uri '{1}', error message '{2}'.", e.SerialId, e.WebRequestUri, e.ErrorMessage);
