...
namespace GameMain
{
    public class NetChannelHelper : INetworkChannelHelper, IReference
    {
        ...
        public int PacketHeaderLength => _netPacketLength + _netCmdIdLength;

        public NetChannelHelper()
        {
        ...
        {
            var packetImpl = packet as CSProtoPacket;
            if (packetImpl == null)
            {
            ...
            _cachedByte.WriteTo(0, packetImpl.ProtoBody.Length);
            _cachedByte.WriteTo(_netPacketLength, packetImpl.Id);
        ...
        public IPacketHeader DeserializePacketHeader(Stream source, out object customErrorData)
        {
            ...
            if (source is MemoryStream memoryStream)
            {
                ...
                var packetSize = bytes.ReadTo(0);
                var packetCmdId = bytes.ReadTo(_netPacketLength);
                packetHeader.PacketLength = packetSize;
                packetHeader.CmdId = packetCmdId;
                Log.Info(Utility.Text.Format("PacketHeader size={0},cmdId={1}", packetSize,
                    packetCmdId));
                return packetHeader;
            }
            return null;
            ...
            out object customErrorData)
        {
            ...
            var scProtoPacket = ReferencePool.Acquire<SCProtoPacket>();
